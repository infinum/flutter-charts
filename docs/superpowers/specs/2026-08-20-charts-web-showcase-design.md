...
One consequence is not cosmetic: `flex_color_picker` 2.6.1 still imports the legacy library throughout, so its widgets cannot resolve a `ThemeData` inside a `material_ui` tree. The app is wrapped in `MaterialUiCompatibilityBridge` via `MaterialApp.builder` for exactly this reason. That bridge is itself `@Deprecated` by design — it is a migration utility — so its single use site is annotated with the reason and removed when the picker migrates.
