...
void main() {
  group('[ChartState] Chart state', () {
    group('[ChartState] Default min value is 0', () {
      test('[Bar] Default min value is 0', () {
        final _state = ChartState.fromList([BarValue<void>(2)]);
        expect(_state.items.isEmpty, false);
        expect(_state.minValue, 0);
      });

      test('[Candle] Default min value is 0', () {
        final _state = ChartState.fromList([CandleValue<void>(5, 2)]);
        expect(_state.items.isEmpty, false);
        expect(_state.minValue, 0);
      });

      test('[Bubble] Default min value is 0', () {
        final _state = ChartState.fromList([BubbleValue<void>(2)]);
        expect(_state.items.isEmpty, false);
        expect(_state.minValue, 0);
      });
    });
    ...
    group('[ChartState] Min value tests', () {
      group('[ChartState] Value min value is item', () {
        test('[Bar] Value min value is item', () {
          final _state = ChartState.fromList([BarValue<void>(2)], options: const ChartOptions(valueAxisMin: 1));
          expect(_state.items.isEmpty, false);
          expect(_state.minValue, 1);
        });

        test('[Candle] Value min value is item', () {
          final _state = ChartState.fromList([CandleValue<void>(5, 2)], options: const ChartOptions(valueAxisMin: 1));
          expect(_state.items.isEmpty, false);
          expect(_state.minValue, 1);
        });

        test('[Bubble] Value min value is item', () {
          final _state = ChartState.fromList([BubbleValue<void>(2)], options: const ChartOptions(valueAxisMin: 1));
          expect(_state.items.isEmpty, false);
          expect(_state.minValue, 1);
        });
      });
      group('[ChartState] Min value is min value', () {
        test('[Bar] Min value is min value', () {
          final _state = ChartState.fromList([BarValue<void>(2)], options: const ChartOptions(valueAxisMin: 10));
          expect(_state.items.isEmpty, false);
          expect(_state.minValue, 2);
        });

        test('[Candle] Min value is min value', () {
          final _state = ChartState.fromList([CandleValue<void>(5, 2)], options: const ChartOptions(valueAxisMin: 10));
          expect(_state.items.isEmpty, false);
          expect(_state.minValue, 2);
        });

        test('[Bubble] Min value is min value', () {
          final _state = ChartState.fromList([BubbleValue<void>(2)], options: const ChartOptions(valueAxisMin: 10));
          expect(_state.items.isEmpty, false);
          expect(_state.minValue, 2);
        });
      });
    ...
    group('[ChartState] Max value tests', () {
      group('[ChartState] Value max value is item', () {
        test('[Bar] Value max value is item', () {
          final _state = ChartState.fromList([BarValue<void>(2)], options: const ChartOptions(valueAxisMax: 1));
          expect(_state.items.isEmpty, false);
          expect(_state.maxValue, 2);
        });

        test('[Candle] Value max value is item', () {
          final _state = ChartState.fromList([CandleValue<void>(5, 2)], options: const ChartOptions(valueAxisMax: 1));
          expect(_state.items.isEmpty, false);
          expect(_state.maxValue, 5);
        });

        test('[Bubble] Value max value is item', () {
          final _state = ChartState.fromList([BubbleValue<void>(2)], options: const ChartOptions(valueAxisMax: 1));
          expect(_state.items.isEmpty, false);
          expect(_state.maxValue, 2);
        });
      });
      group('[ChartState] Max value is max value', () {
        test('[Bar] Max value is max value', () {
          final _state = ChartState.fromList([BarValue<void>(2)], options: const ChartOptions(valueAxisMax: 10));
          expect(_state.items.isEmpty, false);
          expect(_state.maxValue, 10);
        });

        test('[Candle] Max value is max value', () {
          final _state = ChartState.fromList([CandleValue<void>(5, 2)], options: const ChartOptions(valueAxisMax: 10));
          expect(_state.items.isEmpty, false);
          expect(_state.maxValue, 10);
        });

        test('[Bubble] Max value is max value', () {
          final _state = ChartState.fromList([BubbleValue<void>(2)], options: const ChartOptions(valueAxisMax: 10));
          expect(_state.items.isEmpty, false);
          expect(_state.maxValue, 10);
        });
      });
  ...
  group('[Animation] Chart animation', () {
    test('[Animation] Lerp items, min, max', () {
      final _firstState = ChartState.fromList([BarValue<void>(10)]);
    ...
    test('[Animation] Animation animates items', () {
      final _firstState = ChartState.fromList([BarValue<void>(10)]);
      final _secondState = ChartState.fromList([BarValue<void>(20), BarValue<void>(10), BarValue<void>(5)]);
    ...
    test('[Animation] Bar -> Bubble animates different type items', () {
      final _firstState = ChartState.fromList([BarValue<void>(10)]);
      final _secondState = ChartState.fromList([BubbleValue<void>(20)]);
    ...
    test('[Animation] Bubble -> Bar animates different type items', () {
      final _firstState = ChartState.fromList([BubbleValue<void>(10)]);
      final _secondState = ChartState.fromList([BarValue<void>(20)]);
    ...
    test('[Animation] Bar -> Candle animates different type items', () {
      final _firstState = ChartState.fromList([BarValue<void>(10)]);
      final _secondState = ChartState.fromList([CandleValue<void>(20, 10)]);
    ...
    test('[Animation] Candle -> Bar animates different type items', () {
      final _firstState = ChartState.fromList([CandleValue<void>(20, 10)]);
      final _secondState = ChartState.fromList([BarValue<void>(10)]);
    ...
    test('[Animation] Candle -> Bubble animates different type items', () {
      final _firstState = ChartState.fromList([CandleValue<void>(20, 10)]);
      final _secondState = ChartState.fromList([BubbleValue<void>(10)]);
